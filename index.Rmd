...
```{r}
volcanoly(HapMap, genomewideline = -log10(1e-2), effect_size_line = c(-0.5, 0.5))
```

Note that the `volcanoly` function requires an identifier for each point given by the `snp` argument. By default, this is set to `snp = "SNP"`. 
...
```{r}
volcanoly(HapMap, genomewideline = -log10(1e-2), effect_size_line = FALSE)
```
...
```{r}
volcanoly(HapMap, highlight = significantSNP)
```
...
```{r}
volcanoly(HapMap, highlight = FALSE)
```
