...
* This vignette was built with `R markdown` and `knitr`. The source code for this vignette can be found [here](https://raw.githubusercontent.com/sahirbhatnagar/manhattanly/master/vignettes/manhattanly.Rmd).

* In the interactive manhattan, qq and volcano plots below, we only use a subset (random sample of SNPs on chromosomes 5 and 6) of the `HapMap` data included in this package. A more complete version of this vignette can be found at http://sahirbhatnagar.com/manhattanly/
...
Interactive manhattan, Q-Q and volcano plots allow the inspection of specific value (e.g. rs number or gene name) by hovering the mouse over a point, as well as zooming into a region of the
...
library(manhattanly)
set.seed(12345)
HapMap.subset <- subset(HapMap, CHR %in% c(5,6))[sample(1:nrow(subset(HapMap, CHR %in% c(5,6))), 25),]
# for highlighting SNPs of interest
significantSNP <- sample(HapMap.subset$SNP, 5)
head(HapMap.subset)
dim(HapMap.subset)
```
...
```{r, eval=TRUE}
volcanoly(HapMap.subset, snp = "SNP", gene = "GENE")
```
...
HapMap[which(HapMap$SNP %in% significantSNP),]
