...
manhattan_plotly <- function(x, chr = "CHR", bp = "BP", p = "P", snp = "SNP", 
                      col = colorRampPalette(rev(RColorBrewer::brewer.pal(n = 7, name ="Set1")))(nchr),
                      chrlabs = NULL,
                      suggestiveline = -log10(1e-5), genomewideline = -log10(5e-8),
                      suggestiveline_color = "rgb(22, 96, 167)", 
                      genomewideline_color = "rgb(205, 12, 24)",
                      highlight = NULL, logp = TRUE, showlegend = TRUE, ...) {
  
  # # x = gwasResults ;
  # x = subset(gwasResults, CHR %in% c(3,5,6))
  # chr = "CHR" ; bp = "BP" ; p = "P" ; 
  # snp = "SNP" ; 
  # # col = c('rgb(205, 12, 24)', 'rgb(22, 96, 167)');
  # # col = c("#E41A1C", "#FF7F00","#999999");
  ...
  # # highlight = NULL; 
  # highlight = c(snpsOfInterest, paste0("rs6",50:136));
  # 
  # suggestiveline_color = "rgb(22, 96, 167)"; 
  # genomewideline_color = "rgb(205, 12, 24)";
  # genomewideline_color = "green";
  # 
  # logp = TRUE; 
  # annotatePval = NULL ; 
  # annotatePval = 1e-4 ;
  ...
  if (nchr==1) { #If single chromosome, ticks and labels automatic.
    p <- layout(p,              # all of layout's properties: /r/reference/#layout
                ...
                xaxis = list(           # layout's xaxis is a named list. List of valid keys: /r/reference/#layout-xaxis
                  title = xlabel,     # xaxis's title: /r/reference/#layout-xaxis-title
  ...
  if (nchr==1) {
    ...
    p %<>% add_trace(x = d$pos, y = d$logp, 
                     ...
                     marker = list(          # marker is a named list, valid keys: /r/reference/#scatter-marker
                       color = col[1]     # more about marker's "color" attribute: /r/reference/#scatter-marker-color
                     ),
  ...
  # if (suggestiveline) {p %<>% add_trace(x = c(xmin, xmax), 
  #                                       y= c(suggestiveline, suggestiveline), 
  #                                       mode = "lines",
  #                                       color = suggestiveline_color)}
  # if (genomewideline) {p %<>% add_trace(x = c(xmin, xmax), 
  #                                       y= c(genomewideline, genomewideline), 
  #                                       mode = "lines",
  ...
  if (!is.null(highlight)) {
    ...
    p %<>% add_trace(x = d.highlight$pos, y = d.highlight$logp, type = "scatter", 
                     mode = "markers", evaluate = TRUE,
  ...
  # remove annotate snps options... jut show in vignette how this can be 
  ...
  #     
  #     tmp1 <- subset(d, P <= annotatePval)
  #     head(tmp1)
  #     p %<>% layout(annotations = list(x = tmp1$pos, 
  #                                      y = -log10(tmp1$P), 
  #                                      text = tmp1$SNP, 
  #                                      showarrow = T))
  #     str(p)
